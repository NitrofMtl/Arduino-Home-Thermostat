testmodule.controller('temperatureCtrl', ["$scope", "$interval", "ArduinoService", function($scope, $interval, service) {
...
testmodule.service('ArduinoService', ['$http', function($http) {
  return {
    getChannels: function(cb) {
      $http.get('http://192.168.0.110:80/ajax_inputs')
        .success(function(result) {
          cb(null, result);
        });
    },
    switchChannel: function(switchCh, cb) {
      $http.put('http://192.168.0.110:80/switch', {
          switchCh
        })
        .success(function(result) {
          cb(null, true);
    ...
    putChannels: function(channels, cb) {
      $http.put('http://192.168.0.110:80/channels', {
          channels
        })
        .success(function(result) {
          cb(null, true);
